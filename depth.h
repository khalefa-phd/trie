...
private:
  ...
  void AddNextLevelMatching(const string &strPrefixQeury,
    ...
    {
      int d = mtCurrent->btnNode->iDepth + 1 + mtCurrent->iNextQueryIndex - 1 -
              mtCurrent->iMatchingIndex;
      auto lst =
          trie::Index[d][strPrefixQeury[mtCurrent->iNextQueryIndex - 1] - 'a'];
      ...
      vector<BasicTrieNode *>::iterator start = std::lower_bound(
          ...
          [](const BasicTrieNode *element,
             const long value) { return element->iID < value; });

      for (; start != lst.end(); start++) {
        BasicTrieNode *btnNode = *start;
        update_stat();
        if (btnNode->iID > mtCurrent->btnNode->iMaxNodeID)
          break;
        if ((btnNode->iID >= mtCurrent->btnNode->iMinNodeID) &&
            (btnNode->iID <= mtCurrent->btnNode->iMaxNodeID)) {
          int iED = mtCurrent->iED;
          if ((mtCurrent->iNextQueryIndex - 1 - mtCurrent->iMatchingIndex) >
              (btnNode->iDepth - 1 - mtCurrent->btnNode->iDepth))
            iED += (mtCurrent->iNextQueryIndex - 1 - mtCurrent->iMatchingIndex);
          else
            iED += (btnNode->iDepth - 1 - mtCurrent->btnNode->iDepth);

          MatchingTriple *mtNew = new MatchingTriple(mtCurrent->iNextQueryIndex,
                                                     btnNode, iED, bFinished);
  ...
  double getMinString(BasicTrieNode *btn, double dTopkStaticValue, int k) {
    ...
    for (BasicTrieNode *btnqNext : btn->children) {
      ...
      if (dChildStaticValue == I_RESULTS_FINISHED)
        return dChildStaticValue;
    }
  ...
  bool getMinStrings(MatchingTriple *mt, int k) {
    ...
    for (; i < mt->btnNode->lstSortedStringNodes.size(); i++) {
      if (mt->btnNode->lstSortedStringNodes[i]->dStaticValue ==
          mt->dTopkStaticValue) {
        if (results.find(trie::Dictionary[mt->btnNode->lstSortedStringNodes[i]
                                              ->lStringID]) == results.end()) {
          results.insert(trie::Dictionary
                             [mt->btnNode->lstSortedStringNodes[i]->lStringID]);
          if (results.size() == k)
            return true;
        }
      } else
    ...
    if (i == mt->btnNode->lstSortedStringNodes.size())
      return true;
